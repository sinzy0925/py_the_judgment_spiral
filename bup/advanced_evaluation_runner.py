...
def run_search_app(company_name: str, prompt_file: str = None) -> str:
    """gemini_search_app_new_sdk.py を実行してログを返す"""
...
def call_gemini(prompt: str) -> dict:
    """Geminiを呼び出し、レスポンスのJSONをパースして返す"""
    client = genai.Client()
    response = client.models.generate_content(
        model='gemini-2.5-pro',  # 高度な分析のためProモデルを推奨
        contents=prompt
    )
...
def main():
    ...
    try:
        ...
        improved_log = run_search_app(company_name, prompt_file=temp_prompt_file)
        print("再実行完了。")

        # 5. 2回目の評価
        print("\n--- [ステップ5/6] 2回目のログを評価中... ---")
        # 2回目の評価は初回と同じプロンプトで良い
        improved_eval_prompt = create_initial_evaluation_prompt(improved_log)
        improved_eval_result = call_gemini(improved_eval_prompt)
        print("2回目の評価完了。")
        ...
        final_report = call_gemini(comparison_prompt)
